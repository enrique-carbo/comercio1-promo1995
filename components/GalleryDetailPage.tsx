...
const GalleryDetailPage: React.FC<GalleryDetailPageProps> = ({
  album,
  onBack,
}) => {
  ...
  const handlePrev = (e?: React.MouseEvent) => {
    ...
    if (selectedIndex !== null) {
      setSelectedIndex(
        (prevIndex) =>
          (prevIndex! - 1 + album.images.length) % album.images.length
      );
    }
  };
  ...
  const selectedImage =
    selectedIndex !== null ? album.images[selectedIndex] : null;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {album.images.map((image, index) => (
            <div
              ...
              tabIndex={0}
              onKeyDown={(e) =>
                (e.key === "Enter" || e.key === " ") && handleImageClick(index)
              }
            >
              <img
                src={image.src}
              ...
              />
              <p className="font-medium text-brand-text-light mt-2 text-sm truncate">
                {image.alt}
              </p>
            </div>
          ))}
      ...
      {selectedImage && (
        ...
        >
          ...
          >
            ...
            <p
              id="modal-caption"
              className="text-center text-brand-text-light/90 mt-4"
            >
              {selectedImage.alt}
            </p>
          </div>
