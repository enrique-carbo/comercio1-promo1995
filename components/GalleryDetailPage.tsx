  ...
  return (
    <Section id={`gallery-${album.slug}`}>
      <div className="mb-12 grid grid-cols-[auto,1fr,auto] items-center gap-4">
        ...
        {/* Columna derecha: espacio reservado para que el título quede centrado */}
      ...
      {album.images.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {album.images.map((image, index) => (
            <div
              key={index}
              className="bg-brand-light/5 text-center p-2 rounded-lg shadow-md group overflow-hidden"
            >
              <img
                src={image.src}
                alt={image.alt}
              ...
              />
              <p className="font-medium text-brand-text-light mt-2 text-sm">
                {image.alt}
              </p>
            </div>
          ))}
